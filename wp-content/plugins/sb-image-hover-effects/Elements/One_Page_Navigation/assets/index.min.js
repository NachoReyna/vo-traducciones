(function ($) {
    ...
    var SAOnePageNavHandler = function ($scope, $) {
        var onepage_nav_elem = $scope.find('.sa-el-one-page-nav').eq(0);
        ...
        var $section_id = '#' + onepage_nav_elem.data('section-id'),
                ...
                $target_dot = $section_id + ' .sa-el-one-page-nav-item a',
                $nav_item = $section_id + ' .sa-el-one-page-nav-item',
                $active_item = $section_id + ' .sa-el-one-page-nav-item.active';
        ...
        $($target_dot).on('click', function (e) {
            ...
            $($section_id + ' .sa-el-one-page-nav-item').removeClass('active');
        ...
        function updateDot() {
            $('.elementor-section').each(function () {
                ...
                if (($this.offset().top - $(window).height() / 2 < $(window).scrollTop()) && ($this.offset().top >= $(window).scrollTop() || $this.offset().top + $this.height() - $(window).height() / 2 > $(window).scrollTop())) {
                    $($section_id + ' .sa-el-one-page-nav-item a[data-row-id="' + $this.attr('id') + '"]').parent().addClass('active');
                } else {
                    $($section_id + ' .sa-el-one-page-nav-item a[data-row-id="' + $this.attr('id') + '"]').parent().removeClass('active');
                }
            });
        }
        if ($scroll_wheel == 'on') {
    ...
    $(window).on('elementor/frontend/init', function () {
        elementorFrontend.hooks.addAction('frontend/element_ready/sa_el_one_page_navigation.default', SAOnePageNavHandler);
    });
